...
import org.me.web.server.dao.IDepartmentDao;
import org.me.web.server.entity.Department;
import org.me.web.server.service.IDepartmentService;
...
public class DepartmentService implements IDepartmentService {
	...
	public int delById(String id) {
		...
		try {
			...
			if(!"0".equals(dept.getStrPid())){
				count = departmentDao.delByLevel(dept.getStrLevel());
				...
				count = departmentDao.delById(id);
			}
		} catch (Exception e) {
	...
	public int update(Department dept) {
		int count = 0;
		try {
			count = departmentDao.update(dept);
		} catch (Exception e) {
	...
	public List<Department> getList(Department dept) {
		...
		try {
			depts = departmentDao.getListByMap(ObjectUtil.objectToMapIgnoreStatic(dept));
		} catch (Exception e) {
			log.error("get department error : ", e);
